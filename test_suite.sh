...
export REFRAME_ARGS="${REFRAME_ADDITIONAL_ARGS} --nocolor ${REFRAME_NAME_ARGS}"

# List the tests we want to run
echo "Listing tests: reframe ${REFRAME_ARGS} --list"
reframe ${REFRAME_CI_TAG} ${REFRAME_SCALE_TAG} ${REFRAME_ARGS} --list -v
if [[ $? -eq 0 ]]; then
...
reframe ${REFRAME_CI_TAG} ${REFRAME_SCALE_TAG} ${REFRAME_ARGS} --run
reframe_exit_code=$?
