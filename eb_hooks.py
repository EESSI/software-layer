...
def post_postproc_cuda(self, *args, **kwargs):
    ...
    Also prepend path for CUPTI lib64 dir to LIBRARY_PATH.
    """
    ...
    if self.name == 'CUDA' and eessi_installation:
        ...
        # prepend CUPTI/lib64 dir to LIBRARY_PATH
        if 'modluafooter' in self.cfg:
            modluafooter = self.cfg['modluafooter'] + '\n'
        else:
            modluafooter = ''
        print_msg("post_postproc_cuda hook: old modluafooter = '%s'", modluafooter)
        cupti_lib_dir = os.path.join(self.installdir, "extras", "CUPTI", "lib64")
        modluafooter = modluafooter + f'prepend_path("LIBRARY_PATH", {cupti_lib_dir})\n'
        self.cfg['modluafooter'] = modluafooter
        print_msg("post_postproc_cuda hook: new modluafooter = '%s'", self.cfg['modluafooter'])
    else:
