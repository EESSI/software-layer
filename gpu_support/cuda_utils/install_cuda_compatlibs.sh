...
if [[ ${driver_cuda_version%.*} =~ ^[0-9]+$ ]]; then
  if float_greater_than "$driver_cuda_version" "$eessi_cuda_version" ; then
    ...
    if float_greater_than "$eessi_cuda_version" "$cuda_major_minor" ; then
      ...
      if "$TOPDIR"/test_cuda.sh "${required_cuda_version}" ; then
        exit 0
      else
...
if [[ ${file_extension} == "rpm" ]]; then
  ...
  # Restore symlinks
  cd usr/local/cuda-*/compat || fatal_error "Could not 'cd' to $(echo "$PWD"/usr/local/cuda-*/compat)"
  # shellcheck disable=SC2011
  ls ./*.so ./*.so.? | xargs -i -I % sh -c '{ echo -n ln -sf" "; cat %; echo " "%; }'| xargs -i sh -c "{}"
  cd - || fatal_error "Could not 'cd -' from extracted compat dir"
...
cd $host_injections_dir || fatal_error "Could not 'cd' to $host_injections_dir"
...
cd "$current_dir" || fatal_error "Could not 'cd' to $current_dir"
